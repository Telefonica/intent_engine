...
class IntentAssurance():
    ...
    def assure_intent(self, subintent):
        logger.debug("Assurance possible expectations %s",
        ...
        for exp in intent.intentExpectations:
            for schema in get_args(IntentNrm.IntentMncc.__fields__['intentExpectations'].type_):
                try:
                    ...
                    for loc in min_error:
                        ...
                        if len(error_loc)==deepness:
                            for err,i in enumerate(error_loc):
                                if i==0:
